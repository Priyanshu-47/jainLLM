#!/usr/bin/env python3
"""SFT Human Review Server - Task 50: Top-50 Candidates."""
import os, json, http.server, socketserver
from urllib.parse import parse_qs, urlparse
from datetime import datetime

BASE_DIR = "/home/azureuser/jainLLM/svk-corpus"
CANDIDATES_PATH = os.path.join(BASE_DIR, "data/training/sft_review_priority_v1.jsonl")
REVIEW_PATH = os.path.join(BASE_DIR, "data/training/sft_task50_human_review_v1.jsonl")
PORT = 8082
...
class ReviewHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/':
            ...
            c = candidates[idx]
            html = render_task50_example(c, idx, len(candidates), reviews)
            self.send_response(200)
            self.send_header('Content-Type', 'text/html; charset=utf-8')
            self.end_headers()
            self.wfile.write(html.encode('utf-8'))
        elif parsed.path == '/dashboard':
            html = render_task50_dashboard(candidates, reviews)
            self.send_response(200)
            self.send_header('Content-Type', 'text/html; charset=utf-8')
            self.end_headers()
            self.wfile.write(html.encode('utf-8'))
    ...
    def do_POST(self):
        ...
        reviews = load_reviews()
        idx = max(0, min(idx, len(candidates)-1))
        html = render_task50_example(candidates[idx], idx, len(candidates), reviews)
        self.send_response(200)
        self.send_header('Content-Type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
...
def render_task50_example(c, idx, total, reviews):
...
<title>Task 50 Review - {cid}</title>
<style>
...
<li><b>APPROVE</b> if: answers the question, explains rather than cites, understandable, source-grounded, correct attribution, teaches desirable behavior</li>
<li><b>REVISE</b> if: concept is good, source is good, but wording needs correction</li>
<li><b>REJECT</b> if: source doesn't support answer, bad question, wrong attribution, teaches undesirable behavior</li>
</ul></div>
...
def render_task50_dashboard(candidates, reviews):
    ...
    return f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Task 50 Review Dashboard - JainLLM</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
...
<div class="c">
<h1>Task 50: Top-50 SFT Candidate Review</h1>
<p class="sub">JainLLM | 50 priority candidates from Task 49 | Awaiting human review</p>
<div class="warn"><b>Context:</b> These are the top 50 candidates from the 127 generated by the SFT candidate factory. Review for SFT training value.</div>
<div class="stats">
...
if __name__ == '__main__':
    ...
    print(f"Task 50 Review Server starting on http://localhost:{PORT}")
